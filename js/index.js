(function () {
    ...
    $(document).ready(function () {
        charts = renderHomepage3Demo(echarts);
    });
...
function renderHomepage3Demo(echarts) {
    ...
    var dataZoomStartValueTouch = 10;
    var dataZoomEndValue = 45;
    var dataZoomEndValueTouch = 20;
    ...
    var series0Data = [23,32.65,61.06,47.84,54.2,65.96,41.47,48.16,27.59,65.96,52.24,37.22,44.73,53.22,60.08,68.24,58.29,84.78,62.2,71.67,66.78,71.18,46.47,56.39,34.02,47.41,47.11,25.67,28.94,34.73,40.29,46.86,52.96,64.82,51.63,72.33,61.39,66.94,60.57,51.27,47.18,56.49,50.12,52.57,42.45,47.18,34.45,38.69,25.63,35.1,31.51,40.82,30.69,34.45,41.31,46.69,48.82,41.31,52.24,55.51,47.51,32.65,37.71,26.78,16.65,21.06,17.31,26.61,32.98,45.06,41.63,53.39,63.67,58.94,43.92,50.29]; // jshint ignore:line
    ...
    if (window.supportTouch) {
        var dataCount = 30;
        categoryData = categoryData.slice(0, dataCount);
        series0Data = series0Data.slice(0, dataCount);
        series1Data = series1Data.slice(0, dataCount);
        series2Data = series2Data.slice(0, dataCount);
    }
    ...
    function pcChart() {
        var option = {
            ...
            dataZoom: [{
                ...
                handleIcon: 'image://images/index-chart/handle.png', // jshint ignore:line
                handleSize: 20,
                handleStyle: {
                    color: mainColor,
                    shadowBlur: 6,
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    return echarts.util.map([
                        ...
                        return echarts.format.getTooltipMarker(def[1], !idx ? 'border-radius: 0;' : '')
                            + '<span style="display:inline-block;width: 50px;color:' + echarts.format.encodeHTML(def[1]) + '">'
                            + def[2][dataIndex] + (idx ? ' m' : '')
                            + '</span>';
                    }).join('&nbsp;&nbsp;&nbsp;');
                }
            },
    ...
    function touchChart() {
        var optionTouch = {
            ...
            dataZoom: [{
                startValue: dataZoomStartValueTouch,
                endValue: dataZoomEndValueTouch,
                type: 'inside',
                xAxisIndex: [0, 1],
                minValueSpan: minValueSpan,
                zoomOnMouseWheel: 'shift'
        ...
        var myChart2 = echarts.init($('.ch-pc-chart')[0]);
        myChart2.setOption(optionTouch);
    }

    return window.supportTouch ? touchChart() : pcChart();
}
